...
class WhatsappBulkDispatch < ApplicationRecord
  ...
  # `scheduled` sits between draft and processing: confirmed by the operator, waiting for
  # TriggerScheduledItemsJob (every 5 min, same cadence as Campaign's own scheduler) to pick it
  # up once `scheduled_at` arrives — mirrors Campaign#trigger! exactly.
